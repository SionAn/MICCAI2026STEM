"""eNTERFACE'06 EmoBrain (pretraining).

Adapted from NeuroLM (https://github.com/935963004/NeuroLM).
Writes flat files to <out_dir>/EmoBrain/*.pkl; run regroup_subjects.py afterwards.
"""
import argparse
import os
import pickle
from functools import partial
from multiprocessing import Pool
from pathlib import Path

import mne

from common import STANDARD_1020, L_FREQ, H_FREQ, RSFREQ

DATASET_NAME = 'EmoBrain'

# Emotion label of each of the 30 image blocks per session
SESSION_LABEL = {
    'SES1': [0, 1, 0, 2, 1, 2, 0, 1, 2, 0, 2, 1, 1, 0, 2, 0, 1, 0, 0, 1, 0, 2, 1, 2, 2, 0, 1, 1, 2, 2],
    'SES2': [2, 2, 0, 1, 1, 0, 2, 2, 1, 1, 2, 1, 1, 0, 0, 1, 0, 0, 2, 2, 0, 2, 1, 0, 1, 0, 2, 2, 1, 0],
    'SES3': [1, 2, 0, 1, 1, 2, 0, 2, 0, 0, 2, 2, 0, 1, 1, 0, 2, 1, 2, 0, 1, 0, 0, 2, 1, 0, 1, 1, 2, 2]
}


def preprocessing(file_path, l_freq=0.1, h_freq=75.0, sfreq=200):
    raw = mne.io.read_raw_bdf(file_path, preload=False)
    events = mne.find_events(raw)
    raw.drop_channels([ch for ch in raw.ch_names if ch not in STANDARD_1020])

    raw.load_data()
    raw = raw.filter(l_freq=l_freq, h_freq=h_freq, n_jobs=5)
    raw = raw.notch_filter(50.0, n_jobs=5)
    raw = raw.resample(sfreq, n_jobs=5)
    eeg_data = raw.get_data(units='uV')

    return eeg_data, raw.ch_names, events


def process(bdf_file, out_dir):
    print(f'processing {bdf_file.name}')
    eeg_data, ch_order, event = preprocessing(bdf_file, L_FREQ, H_FREQ, RSFREQ)
    time_length = 4 * RSFREQ  # 4-second windows
    name = bdf_file.name.split('.')[0]
    if event.shape[0] not in [30, 31]:
        eeg_data = eeg_data[:, :-10 * RSFREQ]
        for i in range(eeg_data.shape[1] // time_length):
            dump_path = os.path.join(out_dir, DATASET_NAME, f'{DATASET_NAME}_{name}_{i}.pkl')
            pickle.dump(
                {"X": eeg_data[:, i * time_length: (i + 1) * time_length],
                 "ch_names": ch_order},
                open(dump_path, "wb"),
            )
    else:
        event = event[-30:]
        label = SESSION_LABEL[bdf_file.name.split('_')[2]]
        for s in range(event.shape[0]):
            start = int(event[s, 0] / (1024 / RSFREQ)) + 3 * RSFREQ
            end = start + 12.5 * RSFREQ
            for i in range(int(end - start) // time_length):
                dump_path = os.path.join(out_dir, DATASET_NAME, f'{DATASET_NAME}_{name}_{s}_{i}.pkl')
                pickle.dump(
                    {"X": eeg_data[:, start + i * time_length: start + (i + 1) * time_length],
                     "Y": label[s],
                     "ch_names": ch_order},
                    open(dump_path, "wb"),
                )


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--raw_dir', type=str, required=True, help='enterface06_EMOBRAIN/Data/EEG folder (*.bdf)')
    parser.add_argument('--out_dir', type=str, required=True, help='root of the processed datasets')
    parser.add_argument('--num_workers', type=int, default=4)
    args = parser.parse_args()

    os.makedirs(os.path.join(args.out_dir, DATASET_NAME), exist_ok=True)
    group = list(Path(args.raw_dir).rglob('*.bdf'))
    with Pool(processes=args.num_workers) as pool:
        pool.map(partial(process, out_dir=args.out_dir), group)
