"""Trujillo 'Raw EEG Data' and 'Resting State EEG Data' (pretraining, unlabeled).

Writes flat files to <out_dir>/<Raw|Rest>/*.pkl; run regroup_subjects.py afterwards.
"""
import argparse
import os
import pickle
...
def preprocessing_bdf(file_path, l_freq=0.1, h_freq=75.0, sfreq=200):
...
def process(bdf_file, out_dir, dataset_name):
    print(f'processing {bdf_file.name}')
    eeg_data, ch_order = preprocessing_bdf(bdf_file, L_FREQ, H_FREQ, RSFREQ)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, required=True, choices=['Raw', 'Rest'])
    parser.add_argument('--raw_dir', type=str, required=True, help='folder containing the *.bdf files')
